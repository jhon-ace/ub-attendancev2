@if (Auth::user()->hasRole('admin'))
    <x-app-layout>
        <x-user-route-page-name :routeName="'admin.workinghour.index'" />
        <x-content-design>
            <!-- Content Area -->
            <div x-data="{ isFullScreen: (window.innerHeight === screen.height) }" x-init="
                ...
                <div class="relative">
                    <div class="container shadow-lg p-5 sm:p-6 md:p-7 lg:p-8 bg-white rounded-md text-black font-medium"
                        ...
                        <livewire:admin.show-department-working-hours />
                    </div>
                </div>
            </div>
        </x-content-design>
...
@elseif (Auth::user()->hasRole('admin_staff'))
    ...
    <x-app-layout>
        <x-user-route-page-name :routeName="'staff.workinghour.index'" />
        <x-content-design>
            <!-- Content Area -->
            <div x-data="{ isFullScreen: (window.innerHeight === screen.height) }" x-init="
                window.addEventListener('resize', () => {
