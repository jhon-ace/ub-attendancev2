<x-app-layout>
    @if(Auth::user()->hasRole('admin'))
        <x-user-route-page-name :routeName="'admin.attendance.employee_attendance.payroll'" />
    @else
        <x-user-route-page-name :routeName="'admin_staff.attendance.employee_attendance.payroll'" />
    @endif
    <x-content-design>
        <!-- Content Area -->
        <div x-data="{ isFullScreen: (window.innerHeight === screen.height) }" x-init="
            ...
            <div class="relative">
                <div class="container shadow-lg p-5 sm:p-6 md:p-7 lg:p-8 bg-white rounded-md text-black font-medium"
                  ...
                  <livewire:admin.display-datafor-payroll />
                </div>
            </div>
        </div>
    </x-content-design>
