...
class EmployeeChangePass extends Component
{
    use WithPagination;
    ...
    {
         if (Auth::check() && Auth::guard('employee')->user()->school) {
            $this->selectedSchool = Auth::guard('employee')->user()->school->id;
        }

        // $this->selectedDepartment4 = Auth::guard('employee')->user()->department_id;

        $user = Auth::guard('employee')->user();
        $this->selectedDepartment4 = $user->department_id;
    ...
    {   
        ...
        if ($this->selectedMonth && $this->startDate && $this->endDate) {
            ...
            $this->attendancesToShow = EmployeeAttendanceTimeIn::where('employee_id', $this->selectedEmployee)
                ->where('status', '!=', 'Holiday')
                ->whereMonth('check_in_time', $this->selectedMonth)
                ->whereYear('check_in_time', $this->selectedYear) // Filter by selected month
                ->get();

            // Fetch corresponding time-out records
            $this->attendancesToShow = EmployeeAttendanceTimeOut::where('employee_id', $this->selectedEmployee)
                ->where('status', '!=', 'Holiday')
                ->whereMonth('check_out_time', $this->selectedMonth)
                ->whereYear('check_out_time', $this->selectedYear) // Filter by selected month
                ->get();
    ...
    public function render()
    {
        ...
        if ($this->selectedDepartment4) {
            ...
            $employees = Employee::where('department_id', $this->selectedDepartment4)
            ->where('id', $user->id)
            ->get();
        } else {
            $this->departmentToShow = null;
        ...
        return view('livewire.admin.employee-change-pass', [
...
//     public function render()
//     {
//         return view('livewire.admin.employee-attendance-report');
//     }
// }
